...
export default function ViewMode({ isModalActive, setIsModalActive, setMode }: ViewModeProps) {
    ...
    const { setBook, book, bookId, isLoading } = useBook();
                            ...
                            <div className="flex flex-col gap-10 shrink-0 w-[200px]">
                                ...
                                {book.owner == user?.id ? (
                                    <div className="flex flex-col gap-2.5">
                                        <div className="font-inter font-medium py-2.5 border-2 active:scale-[0.99] will-change-transform text-center border-[#2B78B1] text-[#2B78B1] rounded-lg cursor-pointer hover:text-white hover:bg-[#2B78B1] duration-300 select-none" onClick={() => setMode("edit")}>
                                            Edytuj
                                        </div>
                                        {wasButtonPressed ? (
